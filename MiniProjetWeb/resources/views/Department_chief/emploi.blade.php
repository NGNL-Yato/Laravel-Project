    ...
    <div class="filter">
        <select id="salleFilter" style="width:120px;" onchange="location = this.value;">
            ...
            @foreach($salles as $salle)
                <option value="{{ url('Department_chief/emploi/' . $salle->id) }}">{{ $salle->nom_salle }}</option>
            @endforeach
        </select>
    </div>
    <div class="container">
        @if (URL::current() != URL::to('/Department_chief/emploi'))
    ...
    <div class="overlay" style="display: none;">
        <div class="event-modal">
            <div class="add_event hidden-form">
                <form method="POST" action="{{ route('Chief_Department.sceance.store') }}">
                    @csrf
