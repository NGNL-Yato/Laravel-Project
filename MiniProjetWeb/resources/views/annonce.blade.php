...
@section('content')
    <style>
        ...
        .annonce-info table th, .annonce-info table td {
            border: 1px solid #ffffff;
            padding: 10px;
            text-align: left;
        }

        .annonce-info table th {
            background-color: #1e3a8a;
            color: #fff;
        }

        .annonce-info table td {
    ...
    <body class="antialiased full-height">
        ...
        <div class="flex items-center justify-center min-h-screen bg-dots-darker bg-center bg-cover bg-no-repeat bg-fixed" style="background-image: url('{{URL('assets/FST-Tanger.png')}}');">
            <main class="main">
                ...
                <!-- Affichage des informations des annonces -->
<div class="container">
    ...
    @if($annonces->count() > 0)
        @foreach($annonces as $annonce)
            ...
            <div class="annonce-info p-8 rounded-md shadow-md">
                <h2 class="text-2xl font-bold mb-4">{{ $annonce->titre_annonce }}</h2>
                <table class="table">
                    <tbody>
                        @foreach($annonce->getAttributes() as $key => $value)
                            <tr>
                                <th>{{ $key }}</th>
                                <td>{{ $value }}</td>
                            </tr>
